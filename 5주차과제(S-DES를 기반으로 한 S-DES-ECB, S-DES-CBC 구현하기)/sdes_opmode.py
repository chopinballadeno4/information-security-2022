...
- 암호문이 블록의 배수가 되기 때문에 암호화 후 평문을 알기 위해 Padding이 필요
- 각 블록이 독립적으로 동작하므로 한 블록에서 에러가 발생해도 다른 블록에 영향을 주지 않음. 해당 블록까지만 에러 전파한다.
"""
def sdes_encrypt_ecb(text: bitarray, key: bitarray):
    result = bitarray()
    for i in range(0, len(text), 8):
        result = result + sdes(text[i: i+8], key, MODE_ENCRYPT)
...
def sdes_decrypt_ecb(ciphertext: bitarray, key: bitarray):
    ...
    for i in range(0, len(ciphertext), 8):
        result = result + sdes(ciphertext[i: i+8], key, MODE_DECRYPT)
...
- 평문의 각 블록은 XOR 연산을 통해 이전 암호문과 연산되고 첫 번째 암호문에 대해서는 IV가 암호문
대신 사용되며, 이 때 IV는 제 2의 키가 될 수 있다.
...
def sdes_encrypt_cbc(text: bitarray, key: bitarray, iv: bitarray):
    result = bitarray()
    previous_result = sdes(text[0:8] ^ iv, key, MODE_ENCRYPT)
    result = result + previous_result

    for i in range(8, len(text), 8):
        previous_result = sdes(previous_result ^ text[i:i + 8], key, MODE_ENCRYPT)
        result = result + previous_result
...
def sdes_decrypt_cbc(ciphertext: bitarray, key: bitarray, iv: bitarray):
    result = bitarray()
    result = result + sdes(ciphertext[0:8], key, MODE_DECRYPT) ^ iv

    for i in range(8, len(ciphertext), 8):
        result = result + sdes(ciphertext[i:i + 8], key, MODE_DECRYPT) ^ ciphertext[i-8: i]
